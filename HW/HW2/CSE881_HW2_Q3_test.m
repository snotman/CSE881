...
figure;
set(gcf,'color','white');
set(gcf,'Position',[520,85,1020,720]);%resize the plot
for i=1:size(X,1);
...
saveas(gcf,'C:\Users\nan66\Dropbox\CSE881\HW2\digit_image_3a','jpeg')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%(c)
[U,Z,S] = pca(X);
X1=Z(:,1)*diag(S(1))*U(:,1)';%first comp
d1 = sqrt(size(X1,2));
figure;
set(gcf,'color','white');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
X2=Z(:,2)*diag(S(2))*U(:,2)';%second comp
d2 = sqrt(size(X2,2));
figure;
set(gcf,'color','white');
set(gcf,'Position',[520,85,1020,720]);
for i=1:size(X2,1);
subplot(numRows,numCols,i);
img = reshape(X2(i,:),d2,d2)';
imagesc(img); % plot the image
set(gca,'xtick',[]);
set(gca,'ytick',[]);
end;
colormap(gray) ;% convert the images into gray scale
saveas(gcf,'C:\Users\nan66\Dropbox\CSE881\HW2\digit_image_3c','jpeg')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%(d)
figure;
set(gcf,'color','white');
